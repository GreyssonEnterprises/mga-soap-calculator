...
REFERENCE_YAML = Path(__file__).parent.parent.parent / "app" / "data" / "reference-recipes.yaml"
SEED_YAML = Path(__file__).parent.parent.parent / "app" / "data" / "seed-data.yaml"
...
class TestReferenceRecipes:
    ...
    def test_every_recipe_has_required_fields(self, reference_doc: dict) -> None:
        required = {"id", "common_name", "description", "lye", "water", "superfat_percent", "oils"}
    ...
    def test_water_block_is_valid(self, reference_doc: dict) -> None:
        valid_methods = {"percent_of_oils", "lye_concentration", "water_lye_ratio"}
        for recipe in reference_doc["recipes"]:
            water = recipe["water"]
