...
positive_weight = st.decimals(min_value=Decimal("0.1"), max_value=Decimal("10000"), places=2)
...
def test_scaling_preserves_percentages(base_weight, multiplier):
    ...
    for ingredient in original_percentages:
        assert abs(original_percentages[ingredient] - scaled_percentages[ingredient]) < Decimal(
            "0.01"
        )
