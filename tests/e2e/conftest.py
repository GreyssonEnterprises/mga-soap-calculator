...
async def seed_test_data(test_db_engine):
    ...
    Runs for every E2E test using actual seed data
    ...
    async with async_session() as session:
        ...
        for oil_data in OIL_SEED_DATA[:3]:  # Just first 3 for testing
            oil = Oil(**oil_data)
            await session.merge(oil)

        # Seed additives (from production seed data) - use merge for upsert behavior
        for additive_data in ADDITIVE_SEED_DATA[:2]:  # Just first 2 for testing
