...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "fatty_acids", postgresql.JSONB, nullable=False, comment="Percentages of 8 fatty acids"
        ),
        sa.Column(
            "quality_contributions",
            postgresql.JSONB,
            nullable=False,
            comment="Contribution to 7 quality metrics",
        ),
        sa.Column(
            "created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            postgresql.JSONB,
            nullable=False,
            comment="Absolute modifiers to quality metrics at 2% usage",
        ),
        sa.Column(
            "confidence_level",
            sa.String(20),
    ...
    op.create_table(
        ...
        sa.Column(
            "results_data", postgresql.JSONB, nullable=False, comment="Complete calculation results"
        ),
        sa.Column(
            "created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()
        ),
