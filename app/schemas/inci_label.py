...
class OilFormulationItem(BaseModel):
    ...
    oil_id: str = Field(
        ...,
        description="Oil database ID (e.g., 'coconut-oil')",
        min_length=1,
        max_length=50
    )
    weight_grams: float = Field(
        ...,
        description="Weight of oil in grams",
        gt=0,
        le=100000
    )
...
class SoapFormulation(BaseModel):
    ...
    oils: List[OilFormulationItem] = Field(
        ...,
        description="List of oils in the formulation",
        min_length=1,
        max_length=20
    )
...
class InciLabelRequest(BaseModel):
    ...
    formulation: SoapFormulation = Field(
        ...,
        description="Soap formulation details"
    )
    lye_type: str = Field(
        ...,
...
class InciIngredient(BaseModel):
    ...
    oil_id: str = Field(
        ...,
        description="Oil database ID"
    )
    common_name: str = Field(
        ...,
        description="Common name of the oil"
    )
    ...
    percentage: float = Field(
        ...,
        description="Percentage of this ingredient in formulation",
        ge=0,
        le=100
    )
...
class InciLabelResponse(BaseModel):
    ...
    total_oil_weight: float = Field(
        ...,
        description="Total weight of oils in grams"
    )
    lye_type_used: str = Field(
        ...,
        description="Type of lye used: 'naoh' or 'koh'"
    )
...
class IngredientBreakdown(BaseModel):
    ...
    percentage: float = Field(
        ...,
        description="Percentage of total batch weight (to 1 decimal place)",
        ge=0.0,
        le=100.0
    )
    category: str = Field(
        ...,
        description="Ingredient category (water, oil, lye, additive, etc.)"
    )
    notes: Optional[str] = Field(
        None,
        description="Optional notes (e.g., saponification details, warnings)"
    )
...
class ThreeFormatInciResponse(BaseModel):
    ...
    """
    raw_inci: Optional[str] = Field(
        None,
        description="Pre-saponification INCI format (oils + lye listed separately)"
    )
    saponified_inci: Optional[str] = Field(
        None,
        description="Post-saponification INCI format (sodium/potassium salts, lye excluded)"
    )
    common_names: Optional[str] = Field(
        None,
        description="Consumer-friendly common names format"
    )
    ingredients_breakdown: Optional[List[IngredientBreakdown]] = Field(
        None,
        description="Detailed ingredient breakdown with percentages (for verification)"
    )
